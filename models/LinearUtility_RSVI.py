...
class LinearUtility_RSVI:
    def __init__(self, env, transition_probabilities, costs, 
                 vl_slope, num_actions=4, epsilon=0.001, river_flow=None, discount_factor=0.99, QUIET=True) -> None:
    ...
    def __repr__(self):
        if self._env_name == 'RiverProblem':
            self.viz_tools.visualize_V(self, self.V, self._grid_size, 4, self._goal_state, self._i, 
                                str_title=f'Exponential Utility Function - RSMDP - Slope {self._slope}')
    ...
    def calculate_value(self):
        while True and self._i < self._threshold:
            ...
            for S in self.V.keys():
                ...
                for a in range(self._num_actions):
                    ...
                    bellman = self._slope * C + sum(TV)
                    b.append(bellman)
                    
                    if not self.QUIET: print(f'b: {b} | TV: {TV} | C: {C} | Slope: {self._slope}')

                    self._hist_custo[S][a].append(self._slope * C)
                    self._hist_probabilty[S][a].append(sum(TV))
                    self._hist_s0[S][a].append(self._slope * C + T[0] * V[0])
                    self._hist_G[S][a].append(self._slope * C + T[1] * V[1])
        ...
        for S in self.V.keys():
            for a in range(self._num_actions):
                ...
                C = self._cost_function(S, a)
                self.Qi[S][a] = self._slope * C + sum(q)
